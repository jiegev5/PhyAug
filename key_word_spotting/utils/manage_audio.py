...
class AudioPreprocessor(object):
    def __init__(self, sr=16000, n_dct_filters=40, n_mels=40, f_max=4000, f_min=20, n_fft=480, hop_ms=10,tf=[]):
        super().__init__()
        self.n_mels = n_mels
    ...
    def compute_mfccs(self, data):
        ...
        # perform min max normalization to -1 and 1
        # max_ = data.max()
        # min_ = data.min()
        # data = (data - min_)/(max_-min_)
        # data = data*2-1
        # print(f'mean is {data.min()}, max is {data.max()}')
        return data
    ...
    ## added by wenjie, compute mfcc with TF from microphones
    def compute_mfccs_with_tf(self, data):
        # compute spectrum first
        data = np.abs(librosa.stft(data,
                            hop_length=self.hop_length,
                            n_fft=self.n_fft))
        ...
        # print("index = ",index)
        # print("tf is ",self.tf)
        data = data*self.tf[index] # multiply each column of spect with TF
        data = data**2
        # compute mfcc
        data = librosa.feature.melspectrogram(S=data,
                                              sr=self.sr,
    ...
    def compute_mfccs_with_kde(self, data):
        # compute spectrum first
        data = np.abs(librosa.stft(data,
                            hop_length=self.hop_length,
                            n_fft=self.n_fft))
        ...
        # print("index = ",index)
        # print("tf is ",self.tf)
        data = data*new_tf.reshape(241,1) # multiply each column of spect with TF
        data = data**2
        # compute mfcc
        data = librosa.feature.melspectrogram(S=data,
                                              sr=self.sr,
    ...
    def compute_mfccs_with_kde_npy(self, data):
        # compute spectrum first
        # data = np.abs(librosa.stft(data,
        #                     hop_length=self.hop_length,
        #                     n_fft=self.n_fft))
    ...
    def compute_mfccs_inverse_tf(self, data):
        # compute spectrum first
        data = np.abs(librosa.stft(data,
                            hop_length=self.hop_length,
                            n_fft=self.n_fft))
