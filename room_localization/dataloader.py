...
from datagen import load_gtsrb,GTSRB
...
def sample_data_gtsrb():
    # dataset=datasets.MNIST('./data/mnist',train=True,download=True,
    #                transform=transforms.Compose([
    #                    transforms.ToTensor(),
    #                    transforms.Normalize((0.5,0.5,0.5),(0.5,0.5,0.5))
    #                ]))
    # dataset,_=load_gtsrb()
    dataset = GTSRB('train')
    n=len(dataset)
...
def create_target_samples_gtsrb(n=1):
    ...
    #                        transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
    #                    ]))
    # dataset=datasets.MNIST('./data/mnist',train=False,download=False,
    #                transform=transforms.Compose([
    #                    transforms.ToTensor(),
    #                    transforms.Normalize((0.5,0.5,0.5),(0.5,0.5,0.5))
    #                ]))
    ...
    # print(len(dataset))
    # _,dataset=load_gtsrb()
    dataset = dataset = GTSRB('test')
    X,Y=[],[]
    classes=43*[n]
    start = randint(0,len(dataset)*0.8)
    i=0
    while True:
        if len(X)==n*43:
            break
...
def create_target_samples_mnist(n=1):
    # dataset=datasets.SVHN('./data/SVHN', split='train', download=True,
    #                    transform=transforms.Compose([
    #                        transforms.Resize((28,28)),
    #                        transforms.Grayscale(),
...
def create_groups_gtsrb(X_s,Y_s,X_t,Y_t,seed=1):
    #change seed so every time wo get group data will different in source domain,but in target domain, data not change
    torch.manual_seed(1 + seed)
    torch.cuda.manual_seed(1 + seed)
    ...
    for i in range(43):
        for j in range(shot):
            ...
            G2.append((X_s[source_matrix[i][j]],X_t[target_matrix[i][j]]))
            Y2.append((Y_s[source_matrix[i][j]],Y_t[target_matrix[i][j]]))
            G3.append((X_s[source_matrix[i%43][j]],X_s[source_matrix[(i+1)%43][j]]))
            Y3.append((Y_s[source_matrix[i % 43][j]], Y_s[source_matrix[(i + 1) % 43][j]]))
            G4.append((X_s[source_matrix[i%43][j]],X_t[target_matrix[(i+1)%43][j]]))
            Y4.append((Y_s[source_matrix[i % 43][j]], Y_t[target_matrix[(i + 1) % 43][j]]))
...
def sample_groups_gtsrb(X_s,Y_s,X_t,Y_t,seed=1):


    print("Sampling groups")
    return create_groups_gtsrb(X_s,Y_s,X_t,Y_t,seed=seed)
