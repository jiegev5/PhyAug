...
def extract_ABS_feature(data, tf = None, plot=False):
    ...
    if tf is not None:
        ...
        if feature.min() <= 0: # check infinity number
            return
        else:
            for i in range(1): # gen 10 augmented data            
                # assert len(tf) == len(feature)
                gen_tf = tf[0].sample(1)*0.5 + tf[1].sample(1)*0.5 + 1 # add 1 to make sure larger than 0
                # print(gen_tf)
                feature_t = np.multiply(feature,gen_tf)
                feature_t = np.log(feature_t)
                all_feat = np.vstack([all_feat, feature_t]) if all_feat.size else feature_t
...
if __name__ == "__main__":
    ...
    fpost = 'exp_ABS_N4_20_rooms_{}_augmented_kde'.format(source_phone)
    label_index = np.arange(len(train_index))
    feature_extractor = utils.extract_spectrogram()
    # get transfer function
    tf = feature_extractor.get_tf_kde(target_phone[0],target_phone[1],source_phone)
    ...
    for opt in option:
        ...
        for i in range(len(train_index)):
            ...
            for f in files:
                ...
                for j in range(len(splited_array)-1):
                    ...
                    if feat is None: # check infinity number
                        inf_count += 1
                        continue
        ...
        print(f"feature max: {feature.max()}, feature min: {feature.min()}")
        folder = './data/spec'
