...
def eval_plot_confusionMatrix(dataloader, device):
    model = main_models.CNN_SPEC(num_classes=cls_num)
    # summary(model,(1,12,48),batch_size=-1,device='cpu')
    model.to(device)
    # save model
...
if __name__ == "__main__":
    ...
    # files = glob.glob(os.path.join('../data/train', "*.csv"))
    ...
    if retrain:
        ...
        if not os.path.exists(model_dir):
            os.mkdir(model_dir)
        train_dataloader, test_dataloader = load_spec(dataroot='./data/pixel2motor',batch_size=opt['batch_size'])
        train(opt, train_dataloader)
    train_dataloader, test_dataloader = load_spec(dataroot='./data/motorz',batch_size=opt['batch_size'])
    eval_plot_confusionMatrix(test_dataloader,device)
    train_dataloader, test_dataloader = load_spec(dataroot='./data/pixel4',batch_size=opt['batch_size'])
    eval_plot_confusionMatrix(test_dataloader,device)
